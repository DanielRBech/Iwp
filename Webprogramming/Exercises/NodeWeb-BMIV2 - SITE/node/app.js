...
function validateSex(sex){
  if( (sex===maleSex) || (sex===femaleSex) || (sex===indifferentSex) )
    return sex;
...
function validateBMIRecordForm_short(bmiData){
...
function validateBMIRecordForm(bmiData){
  ...
  if(bmiData.has("userName") && bmiData.has("weight") && bmiData.has("height")
    &&bmiData.has("age") && bmiData.has("sex") 
  ){
    let userName=String(bmiData.get("userName"));
    userName=validateUserName(userName);
    let weight=parseInt(bmiData.get("weight"));
...
//will be used by the render functions. That functionality could be moved there.
...
<output>
Your BMI is ${bmiStatus.bmi}. Since last, it has changed ${bmiStatus.delta}. ${bmiStatus.warning?"Beware of your BMI!":""} 
</output>
${renderNavLinks(bmiStatus.userName)}
</section></body>`;
 return page;
}
...
function renderNavLinks(userName){
  let html="<nav>";
  html+=renderAnchor("","Restart | ","/");
  if(userName) {
    html+=renderAnchor("", `See Status page for ${userName} | `, `/bmi-records?userName=${userName}`);
    //NB: it is better to use the SearchParams object to build the query string
  }
  html+=renderAnchor("", "Get Help", "/html/help.html");
  html+="</nav>"
  return html;
}
...
function renderAnchor(attr,text,link){
